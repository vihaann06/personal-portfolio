...
const APP_ICONS = [
  ...
  { id: 'github', label: 'GitHub', src: '/github-icon.png', href: 'https://github.com/vihaann06' },
];
...
const DockButton: React.FC<{
...
}> = ({ label, src, href, onClick, showIndicator }) => {
  const className = 'group relative h-16 w-16 origin-bottom';
  const tooltip = (
  ...
  const icon = <img src={src} alt="" className="h-full w-full object-contain" draggable={false} />;
...
const Dock: React.FC<DockProps> = ({
  ...
  return (
    <div className="pointer-events-none absolute inset-x-0 bottom-2 z-[55] flex justify-center">
      <div
        className="pointer-events-auto flex items-end gap-2.5 rounded-2xl border border-white/25 bg-white/15 px-3 py-2 backdrop-blur-2xl shadow-[0_12px_40px_rgba(0,0,0,0.35)]"
      ...
      >
        ...
        {APP_ICONS.map((app) => (
          <DockButton
            ...
            showIndicator={app.id === 'spotify' ? spotifyActive : undefined}
          />
        ))}
